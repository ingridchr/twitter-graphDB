...
class LoadJSONFriends:
    ...
    def load_graph(self, path):
        ...
        for directory_name in directories:
            ...
            files_list = [f for f in listdir(join(path, directory_name)) if isfile(join(join(path, directory_name), f))]
            # for each file (user)
            for file_name in files_list:
                try:
                    ...
                    if extension == "json":
                            ...
                            user_data = self.get_json(join(join(path, directory_name), file_name))[user_name]
